...
function ExploreContent() {
  ...
  const { data: allUsers, isLoading: isUsersLoading } = useCollection(
    useMemoFirebase(() => firestore ? query(collection(firestore, 'users'), limit(50)) : null, [firestore])
  );
  
  const { data: recentPosts, isLoading: isPostsLoading } = useCollection(
    useMemoFirebase(() => firestore ? query(collection(firestore, 'posts'), orderBy('createdAt', 'desc'), limit(100)) : null, [firestore])
  );
  ...
  if (isUserLoading || !currentUser) return <div className="min-h-screen flex items-center justify-center"><Loader2 className="animate-spin text-primary" /></div>;
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <main className="container mx-auto max-w-2xl pt-12 pb-20 px-0 md:px-4">
        <div className="sticky top-8 z-30 bg-background/80 backdrop-blur-md p-4 border-b border-muted">
          <div className="relative">
            ...
            <Input 
              ...
              className="w-full pr-10 pl-10 h-10 rounded-full bg-secondary/50 border-none text-sm text-right"
              value={searchQuery}
        ...
        {searchQuery ? (
          <div className="mt-2">
            <Tabs defaultValue={searchQuery.startsWith('#') ? "posts" : "users"} className="w-full">
              <TabsList className="w-full justify-start bg-transparent border-b rounded-none h-10 p-0">
                <TabsTrigger value="users" className="rounded-none border-b-2 border-transparent data-[state=active]:border-primary data-[state=active]:bg-transparent px-6 text-xs font-bold">أشخاص</TabsTrigger>
                      ...
                      <Link href={`/profile/${u.id}`} className="flex items-center gap-3">
                        ...
                        <div className="flex flex-col text-right">
                          <div className="flex items-center gap-1.5 justify-end">
                            <VerifiedBadge type={u.verificationType || 'none'} size={14} />
                            <span className="text-sm font-bold text-primary">{u.username}</span>
                          </div>
                          <span className="text-[10px] text-muted-foreground">{u.email}</span>
                        </div>
                      </Link>
                      <Button size="sm" variant={isFollowing ? "outline" : "default"} className="h-8 rounded-full px-5 text-[10px] font-bold" onClick={() => handleFollow(u.id, !!isFollowing)}>
              ...
              <TabsContent value="posts" className="mt-0">
                {isSearching ? (
                  ...
                  <div className="text-center py-20 text-muted-foreground text-xs">لا توجد نتائج مطابقة.</div>
                )}
              </TabsContent>
            </Tabs>
          </div>
          ...
          <div className="divide-y divide-muted">
            <div className="p-4 bg-primary/5 border-b border-primary/10">
              <h2 className="text-xs font-bold text-primary flex items-center gap-2 justify-end">
                متداول الآن في تيمقاد
                <TrendingUp size={14} />
              </h2>
            </div>
              ...
              >
                <div className="flex flex-col text-right">
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    <span className="text-[9px] text-muted-foreground uppercase font-bold tracking-tighter">رائج</span>
                  </div>
                ...
                <ArrowUpRight size={18} className="text-muted-foreground/30 group-hover:text-accent/50 transition-all" />
              </div>
            ))}
          </div>
        )}
