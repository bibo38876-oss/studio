      ...
      <main className="container mx-auto max-w-xl pt-7 pb-20 px-0 md:px-4">
          ...
          {!hasGroup && (
            <Dialog open={isCreateOpen} onOpenChange={setIsCreateOpen}>
              ...
              <DialogContent className="sm:max-w-[425px]">
                <DialogHeader><DialogTitle className="text-sm font-bold">إنشاء مجموعة (واحدة فقط)</DialogTitle></DialogHeader>
                <div className="grid gap-4 py-4">
                  <div className="grid gap-2"><label className="text-[10px] font-bold text-muted-foreground uppercase">اسم المجموعة</label><Input value={groupName} onChange={(e) => setGroupName(e.target.value)} placeholder="اسم المجموعة..." className="h-9 rounded-none bg-secondary/30 border-none text-xs" /></div>
                  <div className="grid gap-2"><label className="text-[10px] font-bold text-muted-foreground uppercase">الوصف</label><Input value={groupDesc} onChange={(e) => setGroupDesc(e.target.value)} placeholder="وصف المجموعة..." className="h-9 rounded-none bg-secondary/30 border-none text-xs" /></div>
                </div>
                <DialogFooter><Button className="w-full rounded-full font-bold text-xs h-9" onClick={handleCreateGroup}>إنشاء الآن</Button></DialogFooter>
              </DialogContent>
            </Dialog>
          )}
        </div>
        ...
        {invites && invites.length > 0 && (
          <div className="p-4 bg-primary/5 border-b">
            <h2 className="text-[10px] font-bold text-primary mb-3 flex items-center gap-1.5"><Bell size={12} /> دعوات انضمام معلقة</h2>
            <div className="space-y-2">
              {invites.map((invite: any) => (
                <Card key={invite.id} className="border-none shadow-none rounded-none bg-card">
                  <CardContent className="p-3 flex items-center justify-between">
                    <div className="flex flex-col"><span className="text-xs font-bold text-primary">{invite.groupName}</span><span className="text-[8px] text-muted-foreground">دعوة من: {invite.invitedBy}</span></div>
                    <div className="flex gap-2">
                      <Button size="icon" variant="ghost" className="h-7 w-7 rounded-full bg-green-50 text-green-600 hover:bg-green-100" onClick={() => updateDocumentNonBlocking(doc(firestore!, 'groups', invite.groupId), { members: arrayUnion(user!.uid), membersCount: (invite.currentMembersCount || 0) + 1 })}><Check size={14} /></Button>
                      <Button size="icon" variant="ghost" className="h-7 w-7 rounded-full bg-red-50 text-red-600 hover:bg-red-100" onClick={() => deleteDocumentNonBlocking(doc(firestore!, 'users', user!.uid, 'groupInvites', invite.id))}><X size={14} /></Button>
                    </div>
                  </CardContent>
                </Card>
              ))}
        ...
        {(isMyGroupsLoading || isJoinedLoading) ? (
          ...
          <div className="divide-y divide-muted">
            {hasGroup && myGroups?.map((group) => (
              <Link key={group.id} href={`/groups/${group.id}`} className="p-4 flex items-center justify-between hover:bg-muted/10 transition-colors group">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 bg-primary/10 rounded-none flex items-center justify-center text-primary border border-primary/20"><Users size={20} /></div>
                  <div className="flex flex-col">
                    <div className="flex items-center gap-2"><span className="text-xs font-bold text-primary group-hover:underline">{group.name}</span><span className="text-[7px] bg-primary/10 text-primary px-1.5 py-0.5 rounded-full font-bold">مجموعتي</span></div>
            ...
            {joinedGroups?.filter(g => g.creatorId !== user?.uid).map((group) => (
              <Link key={group.id} href={`/groups/${group.id}`} className="p-4 flex items-center justify-between hover:bg-muted/10 transition-colors group">
                <div className="flex items-center gap-3">
                  <div className="h-12 w-12 bg-secondary rounded-none flex items-center justify-center text-muted-foreground"><Users size={20} /></div>
                  <div className="flex flex-col"><span className="text-xs font-bold text-primary group-hover:underline">{group.name}</span><span className="text-[10px] text-muted-foreground line-clamp-1">{group.description || 'لا يوجد وصف.'}</span><span className="text-[8px] text-muted-foreground mt-1">{group.membersCount || 0}/100 عضو</span></div>
                </div>
                <ChevronLeft size={16} className="text-muted-foreground/30" />
              </Link>
            ))}
            {(!hasGroup && (!joinedGroups || joinedGroups.length === 0)) && (
              <div className="text-center py-32 px-10 flex flex-col items-center"><MessageSquare size={40} className="text-muted-foreground/20 mb-4" /><p className="text-muted-foreground text-xs font-bold">لا توجد مجموعات بعد.</p><p className="text-[9px] text-muted-foreground mt-1">أنشئ مجموعتك الخاصة وابدأ بدعوة أصدقائك.</p></div>
            )}
          </div>
        )}
      </main>
