...
export default function BottomNav() {
  ...
  const navItems = [
    ...
    { icon: <Bell size={18} />, label: 'تنبيهات', path: '/notifications' },
    { icon: <User size={18} />, label: 'حسابي', path: `/profile/${CURRENT_USER.id}` },
  ];

  return (
    <nav className="fixed bottom-0 left-0 z-50 w-full h-12 bg-background/95 backdrop-blur-md border-t px-2 md:hidden">
            ...
            return (
              <Dialog key={index} open={isPostOpen} onOpenChange={setIsPostOpen}>
                <DialogTrigger asChild>
                  <button className="flex items-center justify-center w-10 h-10 bg-primary text-white rounded-xl shadow-lg -mt-4 transition-transform hover:scale-105 active:scale-95">
                    <Plus size={20} />
                  </button>
                </DialogTrigger>
